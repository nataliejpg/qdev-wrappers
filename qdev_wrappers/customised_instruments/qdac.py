...
class qdac():
    ...
    def defineFunctionGenerator(self, generator, waveform, period, dutycycle=50, repetitions=-1):
        ...
        str = self._checkForError(self._sendReceive(b"fun %d %d %d %f %d" % (generator, waveform, period, dutycycle, repetitions))).decode("utf-8")
        curveType = int(str.split('Curvetype:')[1].split('Period')[0].replace(',', ''))
        period = int(str.split('Period:')[1].split('Dutycycle')[0].replace(',', ''))
        dutyCycle = float(str.split('Dutycycle:')[1].split('Repetitionrate:')[0].replace(',', ''))
        repetitionRate = float(str.split('Repetitionrate:')[1])
        return {'Curvetype': curveType, 'Period': period, "Dutycycle": dutyCycle, "Repetitionrate": repetitionRate}
    ...
    def getPulsetrain(self):
        res = self._checkForError(self._sendReceive(b"pul")).decode("utf-8")
        lowTime=  int(res.split('low time:')[1].split()[0].strip(',').strip('ms'))
        highTime = int(res.split('high time:')[1].split()[0].strip(',').strip('ms'))
        lowValue = float(res.split('low value')[1].split()[0].strip(',').strip('V'))
        highValue =  float(res.split('high value:')[1].split()[0].strip(',').strip('V'))
        pulseCount = int(filter(lambda x:  'times' in x, res.split(','))[0].strip('times').strip())
        return {"LowTime": lowTime, "HighTime": highTime, "LowValue": lowValue, "HighValue": highValue, "PulseCount": pulseCount}
    ...
    def getChannelOutput(self, channel):
        if not self.debugMode:
            self._validateChannel(channel)
        str =  self._checkForError(self._sendReceive(b"wav %d" %channel)).decode("utf-8")
        generator = int (str.split('Generator:')[1].split('Amplitude')[0].replace(',', ''))
        amplitude =  float (str.split('Amplitude:')[1].split('Offset')[0].replace(',', ''))
        offset =  float(str.split('Offset:')[1])
    ...
    def getSyncOutput(self, syncChannel):
        ...
        if syncChannel not in self.syncChannels:
            raise Exception("Invalid sync channel (must be 0-5): %d" % syncChannel)
        return self._checkForError(self._sendReceive(b"syn %d" % syncChannel)).decode("utf-8")
    ...
    def getVoltageCalibration(self, channel, theRange):
        ...
        if theRange == 10.0:
            ...
            raise Exception("Invalid voltage range %f" % theRange)
        str = self._checkForError(self._sendReceive(b"vcal %d %d" % (channel, rangeFlag))).decode("utf-8")
        return {"Gain" :float(str.split('mult')[1].split('offset')[0].strip()), "Offset":float(str.split('offset')[1].strip()) }
    ...
    def getCurrentCalibration(self, channel, theRange):
        ...
        if theRange == 1e-6:
            ...
            raise Exception("Invalid current range: %e" % theRange)
        str =  self._checkForError(self._sendReceive(b"ical %d %d" % (channel, rangeFlag))).decode("utf-8") # TODO: Proper formatting processing of response
        return {"Gain": float(str.split('mult')[1].split('offset')[0].strip()), "Offset": float(str.split('offset')[1].strip())}
