...
class MultiAWG5014Interface(AWGInterface):
    def __init__(self, name, *awg_names):
        awgs = [Instrument.find_instrument(n) for n in awg_names]
        self.CHAN_NUM = 4 * len(awgs)
        self.awgs = awgs
        super().__init__(name)
        for i, awg in enumerate(self.awgs):
            awg.delete_all_waveforms_from_list()
            if i == 0:
                sample_rate = awg.clock_freq()
            for ch in range(4):
                num = (i + 1) * (ch + 1)
                self.submodules[f'ch{num}'].Vpp.source = awg.parameters[f'ch{ch+1}_amp']
        self.sample_rate.set_fn = set_sample_rate
    ...
    def upload(self, forged_sequence: ForgedSequenceType):
        ...
        if self.sequence_mode() == 'element':
            ...
            last_index = self.last_index + 1
            for awg in self.awgs:
                awg.set_sqel_goto_target_index(last_index, 0)
            sleep(self.sleep_time())
        else:
            self.run()
    ...
    def set_repetition_mode(self, rep_mode):
        ...
        if seq_mode == 'element':
            ...
            if rep_mode == 'single':
                # play element once and do not go to the next element after
                for awg in self.awgs:
                    awg.set_sqel_loopcnt_to_inf(index, state=0)
                for awg in self.awgs:
                    awg.set_sqel_goto_target_index(index, 0)
            elif rep_mode == 'inf':
                # play element infinitely and go to the next element after
                for awg in self.awgs:
                    awg.set_sqel_loopcnt_to_inf(index, state=1)
                for awg in self.awgs:
                    awg.set_sqel_goto_target_index(index, index + 1)
        elif seq_mode == 'sequence':
            last_index = self.last_index + 1
            if rep_mode == 'single':
                # at the end of the sequence stop
                ...
                # at the end of the sequence start again
                for awg in self.awgs:
                    awg.set_sqel_goto_target_index(last_index, 1)
        sleep(self.sleep_time())
        if rep_mode == 'inf':
            self.run()
    ...
    def set_element(self, index):
        ...
        if seq_mode == 'sequence':
            ...
            if rep_mode == 'inf':
                ...
                # element needs to be reset to play only once
                for awg in self.awgs:
                    awg.set_sqel_loopcnt_to_inf(old_index, state=0)
            elif rep_mode == 'single':
                # if the previous mode was to play one element once that
                # element needs to be reset to go to the next element after
                for awg in self.awgs:
                    awg.set_sqel_goto_target_index(old_index, old_index + 1)
        ...
        if rep_mode == 'single':
            # do not go to the next element after
            for awg in self.awgs:
                awg.set_sqel_goto_target_index(new_index, 0)
        elif rep_mode == 'inf':
            # play element infinitely
            for awg in self.awgs:
                awg.set_sqel_loopcnt_to_inf(new_index, state=1)
        for awg in self.awgs:
            awg.sequence_pos(new_index)
        self.index = index
        sleep(self.sleep_time())
        if rep_mode == 'inf':
            self.run()
