...
class AnalysisParameter(Parameter):
    """
    Parameter to be used in the analyser which is represented by its name rather
...
class AnalyserBase(Instrument):
    ...
    def __init__(self, name=None):
        ...
        if any([type(v) is not dict for v in [self.MODEL_PARAMETERS,
                                              ...
                                              self.MODEL]]):
            raise RuntimeError('MODEL_PARAMETERS, EXPERIMENTPARAMETERS and '
                               'MODEL dictionaries must be spcified in '
                               'Analyser subclass')
